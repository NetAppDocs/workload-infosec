...
image:workflow-1-discovery.png[Diagram showing how Workload Factory discovers and monitors using read-only AWS API calls.]
...
. Workload Factory issues AWS API request with session permissions.
. Workload Factory receives AWS API response from FSx for ONTAP control plane.
